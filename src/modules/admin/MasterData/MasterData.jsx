...
function MasterData() {
  ...
  try {
    ...
    if (md.activetab === "department") {
      ...
      if (md.editdept) {
        md.setDepartment(prev => prev.map(d => d.id === data.id ? data : d));
    ...
    // ================= ROLE =================
      ...
      if (!md.rname || !md.rcode) return md.swalError("Role name & code required");
      ...
      if (md.editRole) {
        md.setRole(prev => prev.map(r => r.role_id === data.role_id ? data : r));
    ...
    // ================= CATEGORY =================
      ...
      if (md.imageFile) {
        ...
        const up = await fetch(`${BASE_URL}/files/uploadfile`, { method: "POST", body: fd });
      ...
      if (md.editCategory) {
        md.setCategory(prev => prev.map(c => c.category_id === data.category_id ? data : c));
  ...
  return (
    <div className="main-page">
      ...
      <div className="tabs">
        ...
        <span onClick={() => md.setactivetab("department")} className={md.activetab === "department" ? "tab-active" : "tab"}>Department</span>
        <span onClick={() => md.setactivetab("role")} className={md.activetab === "role" ? "tab-active" : "tab"}>Role</span>
        <span onClick={() => md.setactivetab("category")} className={md.activetab === "category" ? "tab-active" : "tab"}>Category</span>
      </div>
      ...
      <div className="main-content">
        ...
        {md.activetab === "department" && <Department dep={md.department} onEdit={d => { md.setEditdept(d); md.setName(d.name); md.setCode(d.code); md.setShowAddModel(true); }} onDelete={deleteDept} />}
        {md.activetab === "role" && <Role roles={md.role} OnEdit={r => { md.setEditRole(r); md.setRname(r.role_name); md.setRcode(r.role_code); md.setShowAddModel(true); }} onDelete={deleteRole} />}
        {md.activetab === "category" && <Category cat={md.category} onEdit={c => { md.setEditCategory(c); md.setCname(c.category_name);md.setImageFile(null); md.setShowAddModel(true); }} onDelete={deleteCategory} />}
      </div>
